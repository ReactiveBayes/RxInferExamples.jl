...
# Syntax highlighting is applied at build time (`prerender=true`) by shelling out to
# `node`, instead of letting `highlight.js` run in the reader's browser. Documenter would
# silently fall back to `prerender=false` if `node` is missing, which ships unhighlighted
# pages, so check explicitly and make that a hard error in CI.
const IS_CI = get(ENV, "CI", nothing) == "true"
const NODE = Sys.which("node")
const PRERENDER = NODE !== nothing

if !PRERENDER
    msg = "No `node` executable found on PATH. Documenter cannot pre-render syntax " *
        "highlighting, code blocks will fall back to client-side `highlight.js`."
    IS_CI ? error(msg) : @warn msg
end
