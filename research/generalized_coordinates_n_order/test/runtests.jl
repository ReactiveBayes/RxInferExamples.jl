...
@testset "GC Car 1D position-only (n-order)" begin
    rng = StableRNG(1)
    n, dt = 200, 0.1
    order = 6
    σ_a, σ_obs_pos = 0.2, 0.5
    x_true, y = GCUtils.generate_gc_car_data(rng, n, dt; order=order, σ_a=σ_a, σ_obs_pos=σ_obs_pos, σ_obs_vel=NaN)

    A, _, Qd = GCUtils.constant_acceleration_ABQ(dt; order=order, σ_a=σ_a)
    Q = Matrix(Qd)
    B = zeros(1, order); B[1,1] = 1.0
    R = Matrix(Diagonal([σ_obs_pos^2]))

    x0_mean = vcat([0.0, 0.0, 0.0], zeros(max(0, order-3)))
    x0_cov  = Matrix(Diagonal(fill(100.0, order)))
    ...
    @test size(mean.(xmarginals)[1], 1) == order
...
@testset "GC Car pos+vel observation (n-order)" begin
    rng = StableRNG(2)
    n, dt = 150, 0.1
    order = 6
    σ_a, σ_obs_pos, σ_obs_vel = 0.2, 0.4, 0.6
    x_true, y = GCUtils.generate_gc_car_data(rng, n, dt; order=order, σ_a=σ_a, σ_obs_pos=σ_obs_pos, σ_obs_vel=σ_obs_vel)

    A, _, Qd = GCUtils.constant_acceleration_ABQ(dt; order=order, σ_a=σ_a)
    Q = Matrix(Qd)
    B = zeros(2, order); B[1,1] = 1.0; B[2,2] = 1.0
