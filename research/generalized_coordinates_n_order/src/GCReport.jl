...
function write_markdown_report(outdir::AbstractString, scen; extra=Dict{Symbol,Any}())
    ...
    if haskey(extra, :metrics)
        met = extra[:metrics]
        println(md, "## Metrics\n")
        D = min(length(get(met, :rmse, [])), length(get(met, :coverage, [])))
        if D > 0
            header_dims = join(["dim_" * string(k) for k in 1:D], " | ")
            println(md, "| metric | $(header_dims) |\n|---|" * "---:|"^D)
            rmse_row = join([@sprintf("%.6f", v) for v in met[:rmse][1:D]], " | ")
            cov_row = join([@sprintf("%.3f", v) for v in met[:coverage][1:D]], " | ")
            println(md, "| rmse | $(rmse_row) |")
            println(md, "| coverage95 | $(cov_row) |\n")
        end
    end
