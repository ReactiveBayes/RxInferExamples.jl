...
"""
Create animated comparison of multiple trials
"""
function create_multi_trial_animation(results::Vector{Any}, output_dir::String="outputs/animations")
    # Ensure output directory exists
    if !isdir(output_dir)
        mkpath(output_dir)
        @info "Created directory" path=output_dir
    end
    ...
    if length(successful_results) < 2
        @warn "Need at least 2 successful trials for animation"
        return
    end

    # Create frames for animation
    frames = []
    n_frames = min(10, length(successful_results))

    # Sample results for animation
    sample_indices = round.(Int, range(1, length(successful_results), length=n_frames))
    ...
    for (i, idx) in enumerate(sample_indices)
        ...
        p = plot(xlabel="θ", ylabel="Density", title="Trial $(idx): n=$(data["n_samples"]), θ=$(data["theta_real"])")

        # Plot prior
        prior = Beta(result["params"]["prior_a"], result["params"]["prior_b"])
        plot!(p, prior, alpha=0.3, color=:blue, label="Prior")

        # Plot posterior
        posterior = Beta(inference["posterior_mean"], 1/inference["posterior_std"]^2 - 1)  # Approximate
        plot!(p, posterior, color=:red, label="Posterior")

        # Add true value
        vline!(p, [data["theta_real"]], color=:black, ls=:dash, label="True θ")
    ...
    # Save animation
    output_file = joinpath(output_dir, "multi_trial_comparison.gif")
    gif(anim, output_file, fps=2)

    println("Multi-trial animation saved to $output_file")
    return anim
end
...
"""
function create_parameter_sweep_plot(results::Vector{Any}, param_x::String, param_y::String, metric::String, output_dir::String="outputs/plots")
    # Ensure output directory exists
    if !isdir(output_dir)
        mkpath(output_dir)
        @info "Created directory" path=output_dir
    ...
    if length(successful_results) == 0
        @warn "No successful results for plotting"
        return
    end

    # Extract data
    x_vals = [get(r["params"], param_x, 0) for r in successful_results]
    y_vals = [get(r["params"], param_y, 0) for r in successful_results]
    metric_vals = [get(r["inference"], metric, 0) for r in successful_results]

    # Create scatter plot
    p = scatter(x_vals, y_vals, zcolor=metric_vals,
                xlabel=param_x, ylabel=param_y, zlabel=metric,
                title="Parameter Sweep: $metric vs $param_x, $param_y",
                colorbar=true, alpha=0.7)

    # Save plot
    output_file = joinpath(output_dir, "parameter_sweep_$(param_x)_$(param_y)_$(metric).png")
    savefig(p, output_file)

    println("Parameter sweep plot saved to $output_file")
    return p
end

"""
Create comprehensive multi-trial dashboard
"""
function create_multi_trial_dashboard(results::Vector{Any}, output_dir::String="outputs/plots")
    # Ensure output directory exists
    if !isdir(output_dir)
        mkpath(output_dir)
        @info "Created directory" path=output_dir
    ...
    if length(successful_results) < 2
        @warn "Need at least 2 successful trials for dashboard"
        return
    end

    # Extract data for plots
    theta_reals = [r["data"]["theta_real"] for r in successful_results]
    posterior_means = [r["inference"]["posterior_mean"] for r in successful_results]
    n_samples = [r["data"]["n_samples"] for r in successful_results]
    kl_divs = [r["statistics"]["kl_divergence"] for r in successful_results]
    exec_times = [r["inference"]["execution_time"] for r in successful_results]

    # Create subplots
    p1 = scatter(theta_reals, posterior_means, xlabel="True θ", ylabel="Posterior Mean",
                 title="Recovery Accuracy", legend=false, alpha=0.6)
    plot!(p1, theta_reals, theta_reals, ls=:dash, color=:red, label="Perfect")

    p2 = scatter(n_samples, kl_divs, xlabel="Sample Size", ylabel="KL Divergence",
                 title="Information Gain", legend=false, alpha=0.6)

    p3 = scatter(n_samples, exec_times, xlabel="Sample Size", ylabel="Execution Time (s)",
                 title="Performance Scaling", legend=false, alpha=0.6)

    p4 = histogram(posterior_means, bins=20, xlabel="Posterior Mean", ylabel="Frequency",
                   title="Posterior Distribution", legend=false)

    # Combine into dashboard
    dashboard = plot(p1, p2, p3, p4, layout=(2, 2), size=(1200, 800))

    # Save dashboard
    output_file = joinpath(output_dir, "multi_trial_dashboard.png")
    savefig(dashboard, output_file)

    println("Multi-trial dashboard saved to $output_file")
    return dashboard
end

"""
Create animated parameter sweep visualization
"""
function create_parameter_sweep_animation(results::Vector{Any}, param::String, metric::String, output_dir::String="outputs/animations")
    # Ensure output directory exists
    if !isdir(output_dir)
        mkpath(output_dir)
        @info "Created directory" path=output_dir
    ...
    sorted_results = sort(successful_results, by=r -> get(r["params"], param, 0))

    # Create frames
    frames = []
    for result in sorted_results
        param_val = get(result["params"], param, 0)
        metric_val = get(result["inference"], metric, 0)

        # Create plot
        p = plot(xlabel=param, ylabel=metric, title="$metric vs $param")

        # Plot all points up to current
        current_idx = findfirst(r -> get(r["params"], param, 0) == param_val, sorted_results)
        current_results = sorted_results[1:current_idx]

        param_vals = [get(r["params"], param, 0) for r in current_results]
        metric_vals = [get(r["inference"], metric, 0) for r in current_results]

        scatter!(p, param_vals, metric_vals, alpha=0.6, color=:blue)
        scatter!(p, [param_val], [metric_val], color=:red, markersize=8)
    ...
    # Save animation
    output_file = joinpath(output_dir, "parameter_sweep_$(param)_$(metric).gif")
    gif(anim, output_file, fps=2)

    println("Parameter sweep animation saved to $output_file")
    return anim
end
